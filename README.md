...
| Week 3 - Enhance           | College + workshops   | Workshops          | Feedback gesprekken(_DONDERDAG_)            |
| Week 4 - Enhance & wrap up | Tweede paasdag        | Individuele vragen | Beoordelingsgesprekken(_DONDERDAG/VRIJDAG_) |
...
2. Fork daarna [deze repository](https://github.com/cmda-minor-web/API-2425) en _clone_ deze op jouw computer.
...
- (Optioneel) Na de installatie is de map `node_modules` aangemaakt, en gevuld met allerlei _packages_. Scroll eens door deze map heen; vele honderden _open source_ ontwikkelaars hebben de packages die je ziet gebouwd en die mag je gratis gebruiken. Ontwikkelen in NodeJS is _standing on the shoulders of giants_.
...
Als het werkt, zet je je server weer uit door in de terminal de toetscombinatie `^c` (control + c) in te voeren. Deze toetsencombinatie wordt in de terminal gebruikt om de huidige taak te stoppen en _controle_ (vandaar de c) terug te krijgen van het programma.
